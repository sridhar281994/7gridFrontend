    ...
    def _on_server_event(self, payload: dict):
        try:
            ...
            if payload.get("status") == "FINISHED":
                self._debug("[SYNC] Backend says FINISHED — stopping game.")
                self._game_active = False
                self._cancel_turn_timer()
                return
            ...
            forfeit = payload.get("forfeit", False)
            ...
            if spawn and actor is not None:
                ...
                # 🔥 FIXED — ALWAYS rotate to backend turn after spawn
                if turn is not None:
                    self._current_player = int(turn)
                else:
                    self._current_player = (actor + 1) % self._num_players
            ...
            if next_turn in forfeited or next_turn not in active:
                next_turn = active[0]
